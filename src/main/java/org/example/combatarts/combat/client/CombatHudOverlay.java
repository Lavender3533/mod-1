...
public class CombatHudOverlay {
    ...
    private static final Identifier LAYER_ID =
            Identifier.fromNamespaceAndPath(CombatArts.MODID, "combat_hud");
    ...
    private static void renderComboCount(GuiGraphics gg, Minecraft mc, ICombatCapability cap, int screenW, int screenH) {
        ...
        if (combo == 99) {
            Component sprint = Component.translatable("hud.combat_arts.combo.sprint");
    ...
    private static void renderDodgeCooldown(GuiGraphics gg, Minecraft mc, ICombatCapability cap, int screenW, int screenH) {
        ...
        Component label = Component.translatable("hud.combat_arts.label.dodge");
    ...
    private static void renderHeavyChargeBar(GuiGraphics gg, Minecraft mc, ICombatCapability cap, int screenW, int screenH) {
        ...
        Component label = atFull
                ? Component.translatable("hud.combat_arts.label.charge_full")
                : Component.translatable("hud.combat_arts.label.charge");
        int labelW = mc.font.width(label);
    ...
    private static Component weaponName(WeaponType type) {
        return switch (type) {
            case SWORD -> Component.translatable("hud.combat_arts.weapon.sword");
            case SPEAR -> Component.translatable("hud.combat_arts.weapon.spear");
            default -> Component.translatable("hud.combat_arts.weapon.unarmed");
        };
    }
    ...
    private static Component stateName(ICombatCapability cap) {
        ...
        return switch (s) {
            case IDLE -> Component.translatable("hud.combat_arts.state.idle");
            case DRAW_WEAPON -> Component.translatable("hud.combat_arts.state.draw");
            case SHEATH_WEAPON -> Component.translatable("hud.combat_arts.state.sheath");
            case ATTACK_LIGHT -> attackLightName(cap);
            case ATTACK_HEAVY -> Component.translatable("hud.combat_arts.state.attack_heavy");
            case ATTACK_HEAVY_CHARGING -> Component.translatable("hud.combat_arts.state.heavy_charging");
            case DODGE -> Component.translatable("hud.combat_arts.state.dodge");
            case BLOCK -> Component.translatable("hud.combat_arts.state.block");
            case PARRY -> Component.translatable("hud.combat_arts.state.parry");
            case INSPECT -> Component.translatable("hud.combat_arts.state.inspect");
            default -> Component.literal(s.name());
        };
    }

    // 轻攻击文字按 武器+combo 走 lang key, 用户改 lang/资源包就能自定义 (例如 "招式一" / "突刺·二").
    // key 形如: hud.combat_arts.combo.sword.1 / hud.combat_arts.combo.spear.2 / hud.combat_arts.combo.sprint
    private static Component attackLightName(ICombatCapability cap) {
        int combo = cap.getComboCount();
        if (combo == 99) return Component.translatable("hud.combat_arts.combo.sprint");
        ...
        if (combo >= 1 && combo <= 3) {
            return Component.translatable("hud.combat_arts.combo." + weaponKey + "." + combo);
        }
        return Component.translatable("hud.combat_arts.state.attack_light");
    }
